[**popyt**](../README.md)

***

[popyt](../modules.md) / Library Exports

# Library Exports

## Classes

- [Caption](classes/Caption.md)
- [Channel](classes/Channel.md)
- [ChannelSection](classes/ChannelSection.md)
- [Comment](classes/Comment.md)
- [Language](classes/Language.md)
- [Playlist](classes/Playlist.md)
- [Region](classes/Region.md)
- [Subscription](classes/Subscription.md)
- [Video](classes/Video.md)
- [VideoAbuseReportReason](classes/VideoAbuseReportReason.md)
- [VideoCategory](classes/VideoCategory.md)
- [VideoRating](classes/VideoRating.md)
- [YouTube](classes/YouTube.md)

## Interfaces

- [IDEntity](interfaces/IDEntity.md)

## Type Aliases

- [Authorization](type-aliases/Authorization.md)
- [Banners](type-aliases/Banners.md)
- [ChannelBrandingSettings](type-aliases/ChannelBrandingSettings.md)
- [ChannelParts](type-aliases/ChannelParts.md)
- [ChannelResolvable](type-aliases/ChannelResolvable.md)
- [ChannelSearchOptions](type-aliases/ChannelSearchOptions.md)
- [ChannelSectionParts](type-aliases/ChannelSectionParts.md)
- [ChannelSectionType](type-aliases/ChannelSectionType.md)
- [CommentParts](type-aliases/CommentParts.md)
- [CommentResolvable](type-aliases/CommentResolvable.md)
- [CommentThreadParts](type-aliases/CommentThreadParts.md)
- [EntitySearchOptions](type-aliases/EntitySearchOptions.md)
- [GenericSearchOptions](type-aliases/GenericSearchOptions.md)
- [Image](type-aliases/Image.md)
- [ItemReturns](type-aliases/ItemReturns.md)
- [ItemTypes](type-aliases/ItemTypes.md)
- [PageOptions](type-aliases/PageOptions.md)
- [PaginatedInstance](type-aliases/PaginatedInstance.md)
- [PaginatedResponse](type-aliases/PaginatedResponse.md)
- [PaginatedType](type-aliases/PaginatedType.md)
- [PlaylistItemParts](type-aliases/PlaylistItemParts.md)
- [PlaylistParts](type-aliases/PlaylistParts.md)
- [PlaylistResolvable](type-aliases/PlaylistResolvable.md)
- [PlaylistSearchOptions](type-aliases/PlaylistSearchOptions.md)
- [ResolutionMap](type-aliases/ResolutionMap.md)
- [Resolvable](type-aliases/Resolvable.md)
- [SearchFilters](type-aliases/SearchFilters.md)
- [SearchType](type-aliases/SearchType.md)
- [SubscriptionParts](type-aliases/SubscriptionParts.md)
- [SubscriptionResolvable](type-aliases/SubscriptionResolvable.md)
- [Thumbnail](type-aliases/Thumbnail.md)
- [VideoCategoryResolvable](type-aliases/VideoCategoryResolvable.md)
- [VideoParts](type-aliases/VideoParts.md)
- [VideoResolvable](type-aliases/VideoResolvable.md)
- [VideoSearchOptions](type-aliases/VideoSearchOptions.md)
- [VideoUpdateResource](type-aliases/VideoUpdateResource.md)
- [YouTubeOptions](type-aliases/YouTubeOptions.md)

## Variables

- [GETTABLE\_CLASSES](variables/GETTABLE_CLASSES.md)

## References

### default

Renames and re-exports [YouTube](classes/YouTube.md)
