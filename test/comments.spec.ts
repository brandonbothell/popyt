...
describe('Comments', () => {
  ...
  it('should not work with valid videos with comments disabled', async () => {
    expect(await youtube.getVideoComments('24EWkH5ipdw').catch(error => { return error }))
      .to.equal('Error fetching items: Error: The video identified by the <code><a href="/youtube/v3/docs/commentThreads/list#videoId">videoId</a></code> parameter has disabled comments.')
  })

  it('should not work with invalid videos', async () => {
    expect(await youtube.getVideoComments('JSFSFDKFaVeryFakeVideoID').catch(error => { return error })).to.equal('Error fetching items: Error: Not found')
  })
