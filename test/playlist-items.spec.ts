...
describe('Playlist items', () => {
  ...
  it('should return an array with a length of <= maxPerPage', async () => {
    expect((await youtube.getPlaylistItems('PLMC9KNkIncKvYin_USF1qoJQnIyMAfRxl', { maxPerPage: 2 }, [ 'id' ])).items.length).to.be.lessThan(3)
  })

  it('should return an array with a size of 50 if maxPerPage isn\'t defined or is < 1', async () => {
    expect((await youtube.getPlaylistItems('PLMC9KNkIncKvYin_USF1qoJQnIyMAfRxl', undefined, [ 'id' ])).items.length).to.equal(50)
  })

  it('should return an array with a size of the playlist if pages is < 1', async () => {
    expect((await youtube.getPlaylistItems('PLMC9KNkIncKvYin_USF1qoJQnIyMAfRxl', { pages: 0 }, [ 'id' ])).items.length).to.be.greaterThan(50)
  })

  it('should return an array with a size of pages * maxPerPage', async () => {
    expect((await youtube.getPlaylistItems('PLMC9KNkIncKvYin_USF1qoJQnIyMAfRxl', { pages: 3, maxPerPage: 5 }, [ 'id' ])).items.length).to.be.equal(15)
  })
})
